...
impl TaskStatsLogger {
    fn new(enabled: bool, stack_refresh_every: Duration, user_task_names: &[&'static str]) -> Self {
        Self {
            enabled,
    ...
    fn snapshot(&mut self, refresh_stack: bool) -> Result<String, TaskStatsError> {
        ...
        for status in statuses.into_iter() {
            ...
            let stack_bytes =
                self.stack_high_water_bytes(status.xHandle, status.eCurrentState, refresh_stack);
        ...
        self.stack_cache.retain(|handle, _| live_handles.contains(handle));
        ...
        if needs_refresh {
            ...
            unsafe {
                sys::vTaskGetInfo(
                    handle,
                    &mut info,
                    1, // xGetFreeStackSpace
                    state_hint,
                );
            }

            let bytes = info.usStackHighWaterMark as usize * core::mem::size_of::<sys::StackType_t>();
